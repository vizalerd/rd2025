import * as THREE from 'three';
        ...
        function trackResourceLoading() {
            ...
            // Если totalResources еще не установлен или равен 0, не обновляем прогресс
            if (totalResources === 0) {
                return;
            }
            ...
            // Прелоадер скрывается только если:
            // 1. Все ресурсы загружены (loadedResources >= totalResources)
            // 2. Все модели загружены (modelsLoaded >= totalModels)
            if (loadedResources >= totalResources && totalResources > 0 && modelsLoaded >= totalModels) {
        ...
        function trackImageLoading() {
            ...
            // Сначала подсчитываем только валидные изображения
            let validImagesCount = 0;
            images.forEach(img => {
                // Пропускаем изображения без src или с пустым src
                if (img.src && img.src !== window.location.href) {
                    validImagesCount++;
                }
            });
            
            // Увеличиваем totalResources только на валидные изображения
            totalResources += validImagesCount;
            
            // Теперь отслеживаем загрузку каждого валидного изображения
            images.forEach(img => {
                // Пропускаем изображения без src или с пустым src
                if (!img.src || img.src === window.location.href) {
                    return;
            ...
            // Проверяем, нужно ли скрывать прелоадер (если все ресурсы уже загружены)
            // Но только если все модели тоже загружены
            if (totalResources > 0 && loadedResources >= totalResources && modelsLoaded >= totalModels) {
    ...
    const perspectiveStrength = 0.02; // Сила эффекта перспективы
        ...
        function handleModelLoaded(object, modelType) {
            ...
            if (modelType === 'yurt') {
                ...
                // Сохраняем изначальную позицию и ротацию при первой загрузке
                // Это позиция, когда юрта еще не двигалась и вид на нее был сверху
                originalYurtPosition.x = 0;
                originalYurtPosition.y = 0;
                originalYurtPosition.z = 0;
                originalYurtRotation.x = 0;
                originalYurtRotation.y = 0;
                originalYurtRotation.z = 0;
    ...
    window.addEventListener('mousemove', function(event) {
        ...
        function initPersonalitiesSlider() {
            ...
            const sliderContainer = sliderTrack ? sliderTrack.closest('.slider-container') : null;
            ...
            // Обработчики свайпа для мыши (drag)
            if (sliderContainer) {
                ...
                document.addEventListener('mousemove', (e) => {
                    ...
                    const newTranslate = initialTranslate + translatePercent;
                    ...
                    sliderTrack.style.transform = `translateX(${clampedTranslate}%)`;
                });
                ...
                document.addEventListener('mouseup', () => {
                    ...
                    const diff = currentX - startX;
                    const containerWidth = sliderContainer.offsetWidth;
                    const swipeThreshold = containerWidth * 0.2; // 20% ширины контейнера
        ...
        function updateFreeSpaceHeight() {
            ...
            if (freeSpace) {
                ...
                const scrollLength = 8100; // Длина скролла в пикселях (должна совпадать с end: "+=7100px")
        ...
        gsap.fromTo('.menu', 
            ...
            },
            {
                ...
                delay: 0.5,
                onComplete: function() {
                    // Исправляем центрирование на мобильной версии
                    if (isMobile && menuElement) {
                        gsap.set('.menu', {
                            xPercent: -50,
                            x: 0,
                        ...
                        menuElement.style.transform = 'translateX(-50%) translateZ(0)';
                    }
                }
            }
        );
        
        // Исправляем позицию меню при изменении размера окна на мобильной версии
        let resizeTimeout;
        window.addEventListener('resize', function() {
            clearTimeout(resizeTimeout);
            resizeTimeout = setTimeout(function() {
                if (window.innerWidth <= 768 && menuElement) {
                    menuElement.style.left = '50%';
                    menuElement.style.transform = 'translateX(-50%) translateZ(0)';
                }
            }, 100);
        ...
        ScrollTrigger.create({
            ...
            onUpdate: (self) => {
                ...
                if (targetTextGlobal) {
                    // Дополнительная проверка: при достижении начала скролла (самого верха) скрываем текст
                    if (scrollPx < 10) {
                    ...
                    } else if (scrollPx < CHECKPOINT_PATH * 0.8) {
                        gsap.to(targetTextGlobal, {
                            opacity: 0,
                            x: 50,
                            duration: 0.3,
                ...
                if (yurtModel) {
                    // Когда скролл в самом начале, принудительно устанавливаем стартовую позицию юрты
                    // Используем позицию с первой загрузки страницы (0, 0, 0) с ротацией (0, 0, 0)
                    if (scrollPx < 10) {
                        // Принудительно устанавливаем стартовую позицию (вид сверху, без наклона)
                        yurtModel.position.x = originalYurtPosition.x;
                        yurtModel.position.y = originalYurtPosition.y;
                        yurtModel.position.z = originalYurtPosition.z;
                        yurtModel.rotation.x = originalYurtRotation.x;
                        yurtModel.rotation.y = originalYurtRotation.y;
                        yurtModel.rotation.z = originalYurtRotation.z;
                    }
                    ...
                    if (scrollPx <= stage1PathEnd) {
                        ...
                        if (targetText) {
                            if (stageProgress >= 0.8 && scrollPx > 200) {
                                gsap.to(targetText, {
                                    opacity: 1,
                                    x: 0,
                                    duration: 0.5,
                        ...
                        // Показываем текст во время паузы (он должен быть виден)
                        const targetText = document.querySelector('.yurt-target-text');
                        if (targetText && scrollPx > 200) {
                            gsap.to(targetText, {
                                opacity: 1,
                                x: 0,
                                duration: 0.3,
        ...
        const bonusQuestions = {
            ...
            6: {
                text: "Летя на электросамокатолете, Ержан увидел не то мираж, не то видение — город будущего, с 4-мя районами, умной энергетикой, экологичный и технологичный. Над ним в небе из облаков сформировалась надпись '2030'. Что увидел Ержан?",
                answers: ["Алматы", "Улытау", "Алатау", "Ержан просто перегрелся"],
                correct: 2
            },
            13: {
                text: "Увлекшись, Ержан начал лететь на своем электросамокатолете слишком высоко, оставляя облака позади. Как гром среди ясного неба разразился голос и спросил, что за место, где человек впервые коснулся неба и ушел к звездам?",
                answers: ["Хан-Тенгри", "Акимат", "«Абу-Даби Плаза»", "Байконур"],
                correct: 3
            },
            19: {
                text: "Уже очень сильно опаздывая, Ержан пришел как раз вовремя. Впопыхах он зашел не в тот подъезд, в соседний. И понял: из него нельзя выйти. На каждом этаже ему говорят 'Куда так рано? Зайди, чай попьем'. Что происходит на самом деле?",
                answers: ["Ержан попал на бесконечное казахское гостеприимство", "Это система фейсконтроля", "Это сон после бешбармака", "Это эксклюзивная экскурсия по обычному подъезду"],
                correct: 0
            }
        };
        ...
        // Функция для полной очистки всех элементов обратной связи
        function clearAllFeedback() {
            const modal = document.getElementById('questionModal');
            if (modal) {
                // Удаляем все элементы .correct-feedback
                const correctFeedbacks = modal.querySelectorAll('.correct-feedback');
                correctFeedbacks.forEach(feedback => feedback.remove());
                
                // Также очищаем из modal-content
                const modalContent = modal.querySelector('.modal-content');
                if (modalContent) {
                    const correctFeedbacksInContent = modalContent.querySelectorAll('.correct-feedback');
                    correctFeedbacksInContent.forEach(feedback => feedback.remove());
                }
                
                // Очищаем answerFeedback
                ...
                // Восстанавливаем видимость кнопок ответов
        ...
        const regionQuestions = {
            ...
            'KZ61': { // Туркестанская область (второй вопрос)
                ...
                url: 'https://cdn.jsdelivr.net/gh/vizalerd/rd2025@latest/kz61.webp',
                answers: [
        ...
        window.openQuestion = function(regionId) {
            ...
            if (modal) {
                ...
                // Добавляем обработчик клика на overlay для закрытия окна
                const handleOverlayClick = (e) => {
                    // Закрываем только если клик был по overlay, а не по modal-content
                    // И только если это не bonus вопрос
                    if (e.target === modal && modal.dataset.isBonus !== 'true') {
                        closeQuestion();
                    }
                };
                ...
                // Полностью очищаем все элементы обратной связи
                clearAllFeedback();
                ...
                if (questionData) {
                    ...
                    if (answers) {
                        ...
                        // Блокируем кнопки, если игра закончена
        ...
        window.checkAnswer = function(regionId, answer, isCorrect) {
            // Проверяем, не закончилась ли игра (жизни <= 0)
            if (gameState.lives <= 0) {
                return; // Не обрабатываем ответы, если игра закончена
            }
        ...
        window.closeRules = function() {
            ...
            if (rulesModal) {
                ...
                // Изменяем высоту game-section на мобильной версии
                const gameSection = document.querySelector('.game-section');
                const isMobile = window.innerWidth <= 480;
                if (gameSection && isMobile) {
                    gameSection.style.height = '50svh';
                    
                    // Автоматически скроллим до карты в центре экрана
                    // Используем небольшую задержку, чтобы браузер успел применить изменение высоты
                    setTimeout(() => {
                        centerMapOnScreen();
                    }, 100);
                }
        ...
        function loseLife() {
            ...
            if (gameState.lives <= 0) {
                // Блокируем все кнопки ответов сразу, чтобы нельзя было нажать после геймовера
                blockAnswerButtons();
        ...
        // Функция блокировки кнопок ответов
        ...
        // Функция разблокировки кнопок ответов
        function unblockAnswerButtons() {
            const answers = document.getElementById('answers');
            if (answers) {
                const buttons = answers.querySelectorAll('.answer-btn');
                buttons.forEach(button => {
        ...
        window.restartGame = function() {
            // Разблокируем скролл страницы (на случай, если он был заблокирован)
            if (document.body) {
            ...
            // Включаем ScrollTrigger обратно
        ...
        function showBonusQuestion(condition) {
            ...
            if (modal) {
                ...
                const modalHeader = modal.querySelector('.modal-header h3');
                if (modalHeader) {
                    modalHeader.textContent = 'Случайное событие';
                }
        ...
        function checkBonusAnswer(condition, answerIndex) {
            ...
            // Полностью очищаем все элементы обратной связи перед обработкой ответа
            clearAllFeedback();
            ...
            if (feedback && feedbackText) {
                ...
                if (isCorrect) {
                    ...
                    // Уменьшаем количество жизней при неправильном ответе
                    loseLife();
        ...
        window.checkGameCompletion = function() {
            // Игра считается завершенной, когда все регионы с вопросами пройдены
            const totalRegionsWithQuestions = Object.keys(regionQuestions).length;
            if (gameState.completedRegions.size >= totalRegionsWithQuestions) {
                // Показываем победное окно
                const winModal = document.getElementById('winModal');
                if (winModal) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Fallback: принудительно скрываем прелоадер через 10 секунд, если он все еще виден
            setTimeout(() => {
            ...
            // Инициализация модального окна "Использованные материалы"
            ...
            if (materialsLink && materialsModal) {
